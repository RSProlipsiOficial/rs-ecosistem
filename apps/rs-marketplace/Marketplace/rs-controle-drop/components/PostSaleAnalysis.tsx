    ...
    monthlyOrders.forEach(order => {
        order.items.forEach(item => {
            ...
            if (order.postSaleEvents && order.postSaleEvents.length > 0) {
                current.eventsCount++;
                // Note: This simplified logic attributes the full refund value to each product in the order.
                // A more complex logic would pro-rate the refund based on item value.
...
const KPICard = ({ title, value, subtext, icon }: any) => (
  <div className="bg-rs-card border border-rs-goldDim/20 p-4 rounded-xl flex flex-col justify-between shadow-sm">
    <div className="flex justify-between items-start mb-2">
       <span className="text-sm text-slate-400 font-medium">{title}</span>
       {icon}
    </div>
    <div>
       <div className="text-2xl font-bold text-slate-100 truncate" title={String(value)}>{value}</div>
