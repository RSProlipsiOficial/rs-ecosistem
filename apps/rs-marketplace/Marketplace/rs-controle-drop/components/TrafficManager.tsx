    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <ModalWrapper isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Editar Registro' : 'Novo Registro de Tráfego'}>
        <form onSubmit={handleSave} className="p-6 space-y-4">
            ...
            <div>
                <label className="label-text">Faturamento Gerado (R$)</label>
                <input type="number" step="0.01" min="0" required value={formData.revenueGenerated} onChange={e => setFormData({...formData, revenueGenerated: parseFloat(e.target.value) || 0})} className="input-field"/>
...
const KPICard = ({ title, value, subtext, icon }: any) => (<div className="bg-rs-card border border-rs-goldDim/20 p-3 rounded-xl flex flex-col justify-between shadow-sm"><div className="flex justify-between items-start mb-2"><span className="text-[10px] uppercase text-slate-500 font-bold tracking-wider">{title}</span>{icon}</div><div><div className="text-lg font-bold text-slate-200 truncate" title={String(value)}>{value}</div>{subtext && <div className="text-[10px] text-slate-500">{subtext}</div>}</div></div>);
...
const TrafficImportModal: React.FC<TrafficImportModalProps> = ({ onClose, onImport }) => {
    ...
    const parseCSV = () => {
        ...
        reader.onload = (e) => {
            ...
            // Simple CSV parser logic (handles basic CSVs)
            const lines = text.split(/\r\n|\n/).filter(line => line.trim() !== '');
            if (lines.length > 0) {
                // Heuristic split: Handle simple commas. 
                // For a production app, use a proper parser library.
                // Here we assume standard CSV without quoted commas for simplicity or handle quotes basically.
    ...
    const parseDate = (dateStr: string) => {
        // Basic parsing based on format selection
        try {
            if (!dateStr) return new Date().toISOString().split('T')[0];
            let year, month, day;
            
            // Clean date string
            dateStr = dateStr.split(' ')[0]; // remove time if exists
    ...
    const parseCurrency = (valStr: string) => {
        ...
        if (clean.includes(',') && clean.includes('.')) {
             ...
             // Assume comma is decimal if it's the only separator, unless it looks like thousands (e.g. 100,000)
    ...
    const handleImport = () => {
        ...
        }).filter(item => item.amountSpent > 0 || item.salesCount > 0); // Filter empty rows
                    ...
                    <div className="flex-1 overflow-y-auto space-y-6">
                        ...
                        <div className="bg-black/20 p-4 rounded-xl border border-white/5 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <MappingField label="Data (Dia)" value={mapping.date} onChange={(v) => setMapping({...mapping, date: v})} options={headers} required />
                                <MappingField label="Valor Gasto (Custo)" value={mapping.amountSpent} onChange={(v) => setMapping({...mapping, amountSpent: v})} options={headers} required />
                                <MappingField label="Quantidade de Vendas/Conversões" value={mapping.salesCount} onChange={(v) => setMapping({...mapping, salesCount: v})} options={headers} />
                                <MappingField label="Valor da Conversão (Receita)" value={mapping.revenueGenerated} onChange={(v) => setMapping({...mapping, revenueGenerated: v})} options={headers} />
                            </div>
                        </div>
...
const MappingField = ({ label, value, onChange, options, required }: any) => (
    <div>
