...
const CheckoutProApp = React.lazy(() => import('./checkout-pro-rs-prólipsi/App'));
...
import './rs-controle-drop/styles.css';
    ...
    const renderAdminContent = () => {
        ...
        switch (view) {
            ...
            case 'rsControleDrop': content = (
                <div className="rs-controle-drop">
                    <Suspense fallback={<div className="p-8 text-center text-gold-400">Carregando Controle Drop...</div>}>
                        <RSControleDropApp />
                    </Suspense>
                </div>
            ); break;
            ...
            <>
                ...
                <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-gray-900 text-gold-400">Carregando Checkout...</div>}>
                    <CheckoutProvider
                        ...
                        orderBump={{
                            id: storeCustomization.orderBump?.productId ?? 'default-bump',
                            name: storeCustomization.orderBump?.title || products.find(p => p.id === storeCustomization.orderBump?.productId)?.name || 'Oferta Especial',
                            description: storeCustomization.orderBump?.description || '',
                            price: storeCustomization.orderBump?.offerPrice ?? 0,
                            originalPrice: products.find(p => p.id === storeCustomization.orderBump?.productId)?.price,
                            image: products.find(p => p.id === storeCustomization.orderBump?.productId)?.images[0] || 'https://via.placeholder.com/150'
                        }}
                    >
                        <CheckoutProApp />
                    </CheckoutProvider>
