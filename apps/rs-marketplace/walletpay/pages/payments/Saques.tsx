...
const Saques: React.FC = () => {
        ...
        const loadData = async () => {
            try {
                ...
                if (pixRes?.data?.success) {
                    setPixKeys(pixRes.data.pix_keys || MOCK_PIX_KEYS);
                } else {
                    setPixKeys(MOCK_PIX_KEYS);
                }
    ...
    const withdrawalHistory = useMemo(() =>
        MOCK_LEDGER_ENTRIES.filter(entry => entry.type === LedgerEventType.WITHDRAWAL || entry.type === LedgerEventType.FEES && entry.description.includes('Saque'))
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-card p-6 sm:p-8 rounded-2xl border border-border">
                    ...
                    <form onSubmit={handleWithdraw} className="space-y-4">
                        ...
                        <div>
                            ...
                            >
                                {MOCK_PIX_KEYS.map(key => <option key={key.id} value={key.id}>{`${key.type.toUpperCase()}: ${key.key}`}{key.isPrimary ? ' (Principal)' : ''}</option>)}
                            </select>
                        </div>
                ...
                <div className="bg-card p-6 rounded-2xl border border-border">
                    ...
                    <div className="space-y-3 mb-4">
                        {MOCK_PIX_KEYS.length > 0 ? MOCK_PIX_KEYS.map(key => (
            ...
            <Modal isOpen={modal.isOpen} onClose={() => setModal({isOpen: false, content: null})} title={modal.content === 'success' ? 'Sucesso!' : 'Adicionar Chave PIX'}>
                ...
                {modal.content === 'addKey' && (
                    <p className="text-center">Gerenciamento de chaves PIX em desenvolvimento.</p>
                )}
            </Modal>
        </div>
    );
