...
const SalesHistory: React.FC<SalesHistoryProps> = ({ orders }) => {
    ...
    const COMMISSION_RATE = 0.05; // 5% de comissão por venda para o CD
    const handleExport = () => {
        const headers = ["ID", "Data", "Hora", "Consultor", "Patrocinador", "Placa", "Status", "Total (R$)", "Pontos VP", "Comissão CD (R$)"];
        const rows = filteredOrders.map(o => [
            ...
            o.total.toFixed(2),
            o.totalPoints || 0,
            (o.total * COMMISSION_RATE).toFixed(2)
        ].join(","));
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto items-end">
                    <div className="flex gap-2">
                        {/* Date Filters */}
                        <div className="flex gap-2 items-center bg-dark-950 p-1 rounded-lg border border-dark-800">
                            <input
                                type="date"
                                className="bg-dark-900 border-none text-white px-3 py-1.5 rounded focus:outline-none focus:ring-1 focus:ring-gold-400 text-sm h-8"
                                value={startDate}
                            ...
                            {filteredOrders.length === 0 ? (
                                ...
                                filteredOrders.map(order => (
                                    <tr key={order.id} className="hover:bg-dark-800/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase ${normalizeStatus(order.status) === 'ENTREGUE' || normalizeStatus(order.status) === 'CONCLUIDO' ? 'bg-green-900/20 text-green-500' :
                                                normalizeStatus(order.status) === 'PENDENTE' ? 'bg-yellow-900/20 text-yellow-500' :
