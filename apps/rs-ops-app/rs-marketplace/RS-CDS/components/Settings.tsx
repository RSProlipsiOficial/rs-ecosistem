...
const Settings: React.FC<SettingsProps> = ({ initialData, onSaveProfile, cdId }) => {
  const [activeTab, setActiveTab] = useState<'PROFILE' | 'BANK' | 'PAYMENT' | 'SHIPPING' | 'SYNC'>('PROFILE');

  // Initialize state with props or fallback to defaults
  const [settings, setSettings] = useState<SettingsData>(initialData || defaultSettings);
    ...
    <div className="animate-fade-in space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 border-b border-dark-800">
        {[
          ...
          { id: 'SYNC', label: 'Sincronização', icon: <RefreshCw size={18} /> }
        ].map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id as any)}
            className={`flex items-center gap-2 px-6 py-3 rounded-t-lg text-sm font-bold transition-all whitespace-nowrap ${activeTab === tab.id
