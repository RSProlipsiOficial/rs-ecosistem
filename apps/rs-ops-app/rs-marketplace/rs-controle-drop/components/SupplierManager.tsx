    ...
    },
    {
      ...
      render: (s) => (
        <div className="flex justify-center gap-2">
            ...
            {currentUser.role === 'Admin' && <button onClick={() => onDelete(s.id)} className="p-2 text-slate-400 hover:text-red-400"><Trash2 size={16}/></button>}
        </div>
      )
    }
  ];
...
interface SupplierMetricsModalProps {
  supplier: Supplier;
  orders: Order[];
  onClose: () => void;
}

const SupplierMetricsModal: React.FC<SupplierMetricsModalProps> = ({ supplier, orders, onClose }) => {
