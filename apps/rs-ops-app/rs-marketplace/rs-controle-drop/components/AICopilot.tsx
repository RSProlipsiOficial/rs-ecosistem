...
import { User, ChatMessage, MonthlySummary, Order, Product, TrafficSpend, Lead, Customer, Cart, Checkout, AbandonmentLog, MessageTemplate, Supplier, AppAlert, ProductSupplier, AdCreativeSuggestion, CustomerSegment } from '../types';
import { Send, Bot, User as UserIcon, Loader2, Zap, BarChart2, DollarSign, ShoppingBag, AlertOctagon, Terminal, ExternalLink, HelpCircle, MessageCircle, ArrowRight, ShieldAlert, CheckCircle, XCircle, Clock, Plus, Trash2, Copy, Image as ImageIcon, Pilcrow, Edit3 } from 'lucide-react';
...
const readTools: FunctionDeclaration[] = [
    {
        ...
        parameters: {
            type: Type.OBJECT,
            properties: {
                destination: { type: Type.STRING, description: 'ID da aba: dashboard, orders, products, customers, suppliers, recovery, funnel-monitor, traffic, settings, price-optimizer, crm-clientes.' },
                params: { 
                    type: Type.OBJECT, 
    ...
    {
        name: 'analyze_conversion_rate',
        description: 'Analisa a taxa de conversão de vendas em um período e compara com o período anterior. Ajuda a detectar quedas de performance.',
        parameters: {
            type: Type.OBJECT,
            properties: {
                period: { type: Type.STRING, description: "O período a ser analisado: 'week' (últimos 7 dias) ou 'month' (últimos 30 dias)." },
            },
            required: ['period']
        }
    },
    {
        name: 'analyze_checkout_abandonment_funnel',
        description: 'Analisa as etapas do funil de checkout para identificar onde os clientes mais abandonam (dados pessoais, frete, pagamento).',
        parameters: {
            type: Type.OBJECT,
            properties: {
                period: { type: Type.STRING, description: "O período a ser analisado: 'week' (últimos 7 dias) ou 'month' (últimos 30 dias)." },
    ...
    {
        name: 'summarize_pricing_opportunities',
        description: 'Analisa todos os produtos para encontrar oportunidades de otimização de preço e resume os resultados.',
        parameters: { type: Type.OBJECT, properties: {} }
    },
    {
        name: 'generate_ad_creative',
        description: 'Busca informações de um produto para usar como base para criar anúncios.',
        parameters: {
            type: Type.OBJECT,
            properties: {
                product_name: { type: Type.STRING, description: 'Nome do produto para o qual os anúncios serão criados.' }
            },
    ...
    },
    {
        ...
        description: 'Lista clientes de um segmento específico (VIPs, Em Risco, etc.) com base na análise de RFM (Recência, Frequência, Valor).',
        parameters: {
            type: Type.OBJECT,
            properties: {
                segment: { type: Type.STRING, description: "O segmento de cliente a ser buscado. Valores possíveis: 'VIPs', 'Campeões', 'Leais', 'Potencial', 'Em Risco', 'Hibernando', 'Novos'." },
                limit: { type: Type.NUMBER, description: 'Número de clientes a retornar (padrão 5).' }
            },
            required: ['segment']
        }
    }
...
export const AICopilot: React.FC<AICopilotProps> = ({ 
    currentUser, monthlySummary, orders, products, traffic, leads, customers, carts, checkouts, abandonmentLogs, templates, suppliers, alerts, productSuppliers,
    ...
    const handleConfirmAction = async (confirmed: boolean) => {
        ...
        setIsLoading(true);
        // FIX: Added 'customers' to aiContext to match the updated AIContext interface.
        const aiContext: AIContext = { 
            orders, products, traffic, carts, checkouts, abandonmentLogs, templates, suppliers, productSuppliers, customers,
    ...
    const handleSendMessage = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            Você é o RS.AI, um consultor, analista de dados SÊNIOR, ANALISTA DE CRM, ANALISTA DE CONVERSÃO, ANALISTA DE PREÇOS e DIRETOR DE CRIAÇÃO para E-commerce (Dropshipping).
            Seu foco é encontrar problemas e oportunidades para aumentar as vendas e o lucro.

            CONTEXTO:
            - Tópicos Recentes: ${userMemory.recentTopics.join(', ')}
            - Últimas Ações: ${JSON.stringify(userMemory.lastActions.slice(0, 2))}
            
            MISSÃO: Analisar, Diagnosticar, Otimizar e EXECUTAR. Seja proativo em encontrar problemas de conversão, oportunidades de preço e analisar segmentos de clientes.
            ...
            - Se o usuário perguntar sobre conversão ou performance, use as ferramentas 'analyze_conversion_rate' e 'analyze_checkout_abandonment_funnel' para obter dados concretos.
            - Se o usuário perguntar sobre preços, use a ferramenta 'summarize_pricing_opportunities' e recomende a navegação para a tela de otimização.
            - Para criar anúncios, use a ferramenta 'generate_ad_creative' para obter dados do produto primeiro.
            - Se o usuário perguntar sobre clientes ou segmentos, use a ferramenta 'get_customer_segment'.
            - Forneça insights claros e recomendações acionáveis.
            ...
            if (functionCalls && functionCalls.length > 0) {
                for (const call of functionCalls) {
                    ...
                    // --- SPECIAL CASE: AD CREATIVE (2-step call) ---
                    if (name === 'generate_ad_creative') {
                        const productData = aiDataService.generateAdCreative(aiContext, args as any);
                        if (productData.error) {
                            finalResponseText = productData.error;
                        } else {
                            const creativePrompt = `Com base nestes dados do produto: ${JSON.stringify(productData)}, e no pedido do usuário: "${input}", gere um pacote de criativos para anúncios. Inclua 3 headlines, 2 copies e 2 ideias de imagem/vídeo.`;
                                ...
                                config: {
                                    ...
                                    responseSchema: {
                                        ...
                                        properties: {
                                            ...
                                            imageInstructions: { type: Type.ARRAY, items: { type: Type.STRING } },
                                        },
                                        required: ['headlines', 'copies', 'imageInstructions']
                                    }
                                }
                            });
                            ...
                            try {
                                ...
                                setMessages(prev => [...prev, {
                                    id: crypto.randomUUID(),
                                    role: 'model',
                                    content: 'Aqui estão algumas sugestões de criativos para seu produto:',
                                    timestamp: new Date().toISOString(),
                                    uiType: 'ad_creative_list',
                                    data: creativeData,
                                }]);
                            } catch (e) {
                                console.error("Error parsing ad creative JSON", e);
                                setMessages(prev => [...prev, { id: crypto.randomUUID(), role: 'model', content: "Não consegui gerar os criativos em um formato válido.", timestamp: new Date().toISOString() }]);
                            }
                        }
                        setIsLoading(false);
                        return; // Exit early
                    ...
                    let displayMessage = "";
                    let isAction = false;
                    let uiType: ChatMessage['uiType'] = 'default';
                    let uiData: any = null;
                    ...
                    if (name === 'navigate') {
                    ...
                    else if (['get_products', 'get_orders', 'list_abandoned_carts', 'analyze_abandonment', 'get_marketing_stats', 'get_financial_metrics', 'get_supplier_metrics', 'analyze_conversion_rate', 'analyze_checkout_abandonment_funnel', 'summarize_pricing_opportunities', 'get_customer_segment'].includes(name)) {
                        if (name === 'list_abandoned_carts') {
                            ...
                            // @ts-ignore
                            result = aiDataService[name === 'get_products' ? 'getProducts' : name === 'get_orders' ? 'getOrders' : name === 'analyze_abandonment' ? 'analyzeAbandonment' : name === 'get_marketing_stats' ? 'getMarketingStats' : name === 'get_financial_metrics' ? 'getFinancialMetrics' : name === 'get_supplier_metrics' ? 'getSupplierMetrics' : name === 'analyze_conversion_rate' ? 'analyzeConversionRate' : name === 'summarize_pricing_opportunities' ? 'summarizePricingOpportunities' : name === 'get_customer_segment' ? 'getCustomerSegment' : 'analyzeCheckoutAbandonmentFunnel'](aiContext, args as any);
                        ...
                        <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[85%] flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                ...
                                <div className="flex flex-col gap-2 w-full">
                                    ...
                                    {/* Rich UI: Ad Creative List */}
                                    {msg.uiType === 'ad_creative_list' && msg.data && (
                                        <AdCreativeDisplay data={msg.data} />
                                    )}
                                </div>
                            </div>
                        </div>
                ...
                <div className="p-4 border-t border-white/10 bg-rs-card">
                    <form onSubmit={handleSendMessage} className="relative">
                        <input 
                            ...
                            placeholder={pendingAction ? "Confirme a ação acima..." : "Digite sua mensagem (ex: 'Liste meus clientes VIP')"}
                    ...
                    <div className="mt-3 flex gap-2 overflow-x-auto text-[10px] text-slate-500 pb-1 scrollbar-hide">
                        ...
                        <SuggestionChip icon={<ShoppingBag size={12}/>} text="Produtos Problemáticos" onClick={() => setInput("Quais produtos são mais abandonados?")} />
...
// --- Rich UI Components ---
const AdCreativeDisplay: React.FC<{ data: AdCreativeSuggestion }> = ({ data }) => {
    ...
    return (
        <div className="bg-white/5 p-4 rounded-xl border border-white/10 space-y-4 animate-fade-in">
            <CreativeSection icon={<Edit3 size={16} />} title="Títulos (Headlines)">
                {data.headlines.map((text, i) => (
                    <CopyableText key={i} text={text} onCopy={copyToClipboard} />
                ))}
            </CreativeSection>
            <CreativeSection icon={<Pilcrow size={16} />} title="Textos Principais (Copy)">
                {data.copies.map((text, i) => (
                    <CopyableText key={i} text={text} onCopy={copyToClipboard} />
                ))}
            </CreativeSection>
            <CreativeSection icon={<ImageIcon size={16} />} title="Ideias de Imagem/Vídeo">
                {data.imageInstructions.map((text, i) => (
                    <CopyableText key={i} text={text} onCopy={copyToClipboard} />
                ))}
            </CreativeSection>
        </div>
...
const CreativeSection: React.FC<{ icon: React.ReactNode, title: string, children: React.ReactNode }> = ({ icon, title, children }) => (
    <div>
        <h4 className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{icon} {title}</h4>
        <div className="space-y-2">{children}</div>
    </div>
);

const CopyableText: React.FC<{ text: string, onCopy: (text: string) => void }> = ({ text, onCopy }) => (
    <div className="group flex justify-between items-start gap-2 bg-black/20 p-2 rounded-lg border border-transparent hover:border-white/10">
        <p className="text-sm text-slate-300">{text}</p>
        <button onClick={() => onCopy(text)} className="p-1 text-slate-500 hover:text-rs-gold opacity-0 group-hover:opacity-100 transition-opacity" title="Copiar">
            <Copy size={14} />
        </button>
    </div>
);
