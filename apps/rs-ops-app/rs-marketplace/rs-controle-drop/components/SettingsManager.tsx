...
import { PaymentMethodConfig, ShippingConfig, AuditLog, MessageTemplate, AovBoostConfig } from '../types';
import { Settings, Save, X, Plus, Edit2, Trash2, CreditCard, Truck, ShieldCheck, Search, Clock, MessageSquare, Info, TrendingUp } from 'lucide-react';
...
interface SettingsManagerProps {
  ...
  onAovBoostConfigChange: (config: AovBoostConfig) => void;
}
...
export const SettingsManager: React.FC<SettingsManagerProps> = ({ 
    ...
    aovBoostConfig, onAovBoostConfigChange
}) => {
    ...
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      {activeTab === 'general' && (
        <div className="space-y-8 animate-fade-in">
             {/* AOV Boost Settings */}
            <ConfigSection 
                title="Incentivos de Venda (AOV Boost)" 
                icon={<TrendingUp size={20}/>}
                hideAddButton={true}
            >
                <div className="p-4 space-y-3">
                    <div>
                        <label className="label-text">Valor para Frete Grátis (R$)</label>
                        <input 
                            type="number" 
                            value={aovBoostConfig.freeShippingThreshold}
                            onChange={e => onAovBoostConfigChange({ ...aovBoostConfig, freeShippingThreshold: parseFloat(e.target.value) || 0 })}
                            className="input-field"
                            placeholder="Ex: 250"
                        />
                        <p className="text-xs text-slate-500 mt-1">Defina o valor mínimo no carrinho para o cliente ganhar frete grátis.</p>
                    </div>
                </div>
            </ConfigSection>
...
const ConfigSection: React.FC<ConfigSectionProps> = ({ title, icon, onAddNew, children, hideAddButton = false }) => (
  <div className="bg-rs-card rounded-xl border border-rs-goldDim/20 overflow-hidden shadow-lg">
    <div className="p-4 border-b border-white/5 flex justify-between items-center">
      ...
      {!hideAddButton && onAddNew && <button onClick={onAddNew} className="btn-primary-sm flex items-center gap-1"><Plus size={16}/> Adicionar</button>}
    </div>
    {children}
...
const ConfigTable = <T extends { id: string }>({ headers, data, renderRow }: ConfigTableProps<T>) => (
  <div className="overflow-x-auto">
    <table className="w-full text-left text-sm">
      ...
      <tbody className="divide-y divide-white/5 text-slate-400">
        {data.map((item) => <tr key={item.id} className="hover:bg-white/5">{renderRow(item)}</tr>)}
      </tbody>
    </table>
  </div>
);
...
interface PaymentConfigModalProps {
    ...
    onSave: (data: Omit<PaymentMethodConfig, 'id' | 'userId'>) => void;
    onClose: () => void;
}
...
interface ShippingConfigModalProps {
    ...
    onSave: (data: Omit<ShippingConfig, 'id' | 'userId'>) => void;
    onClose: () => void;
}

const ShippingConfigModal: React.FC<ShippingConfigModalProps> = ({ config, onSave, onClose }) => {
...
interface TemplateModalProps {
  ...
  onSave: (t: Omit<MessageTemplate, 'id'>) => void;
  onClose: () => void;
}
