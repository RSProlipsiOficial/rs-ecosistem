...
import { Cart, Checkout, User, CheckoutFunnelStep, Product, CartItem, AovBoostConfig } from '../types';
...
export const FunnelMonitor: React.FC<FunnelMonitorProps> = ({ currentUser, users, products, aovBoostConfig, initialFilter }) => {
...
const CartDetailModal: React.FC<CartDetailModalProps> = ({ cart, products, aovBoostConfig, onClose, onUpdateItemQuantity, onRemoveItem, onStartCheckout }) => {
    ...
    const freeShippingThreshold = aovBoostConfig?.freeShippingThreshold || 99999;
    const missingForFreeShipping = freeShippingThreshold - subtotal;
    const freeShippingProgress = Math.max(0, Math.min(100, (subtotal / freeShippingThreshold) * 100));
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 overflow-y-auto">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {missingForFreeShipping > 0 ? (
                        <div className="bg-emerald-900/20 border border-emerald-500/30 p-4 rounded-lg text-center">
                            <div className="flex items-center gap-2 text-xs text-emerald-300 mb-2">
                                <Gift size={16}/> Faltam <span className="font-bold text-white">R$ {missingForFreeShipping.toFixed(2)}</span> para você ganhar Frete Grátis!
                            </div>
                            <div className="w-full bg-black/30 rounded-full h-2.5">
                                <div className="bg-emerald-500 h-2.5 rounded-full" style={{ width: `${freeShippingProgress}%` }}></div>
                            </div>
                        </div>
                    ) : (
                         <div className="bg-emerald-900/20 border border-emerald-500/30 p-4 rounded-lg text-center flex items-center justify-center gap-2 text-emerald-300 font-bold">
                            <Gift size={16}/> Parabéns! Você ganhou Frete Grátis!
                        </div>
                    )}
                    ...
                    <div className="space-y-4">
                        {cart.items.map(item => {
                            ...
                            let savings = 0;
                            if (isBundle && product.bundleConfig) {
                                const sumOfItems = product.bundleConfig.items.reduce((acc, bundleItem) => {
                                    const p = products.find(p => p.id === bundleItem.productId);
                                    return acc + (p ? p.salePrice * bundleItem.quantity : 0);
                                }, 0);
                                savings = sumOfItems - product.salePrice;
                            }
                            return (
                                <div key={item.id} className="flex items-center gap-4 bg-black/20 p-3 rounded-lg border border-white/5">
                                    ...
                                    <div className="flex-1">
                                        <div className="font-bold text-slate-200 flex items-center gap-2">
                                            {item.productName}
                                            {isBundle && <span className="text-xs bg-rs-gold text-rs-black font-bold px-2 py-0.5 rounded">KIT</span>}
                                        </div>
                                        {isBundle && savings > 0 && (
                                            <div className="text-xs text-emerald-400 font-medium mt-1">Economia de R$ {savings.toFixed(2)}</div>
                                        )}
                                        <div className="text-xs text-slate-500 mt-1">Preço: R$ {item.unitPrice.toFixed(2)}</div>
                                    </div>
                    ...
                    {recommendedProducts.length > 0 && (
                        <div>
                            <h4 className="font-bold text-slate-300 flex items-center gap-2 mb-3"><PackageIcon size={16}/> Leve também</h4>
                ...
                <div className="space-y-6 bg-black/20 p-6 rounded-xl border border-white/10 h-fit">
                    ...
                    <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between"><span className="text-slate-400">Frete</span><span>{shippingCost !== null ? `R$ ${shippingCost.toFixed(2)}` : 'A calcular'}</span></div>
