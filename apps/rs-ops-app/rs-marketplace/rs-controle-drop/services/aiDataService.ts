import { Order, Product, TrafficSpend, Cart, Checkout, AbandonmentLog, MessageTemplate, Supplier, AIUserContext, ChatMessage, ChatSession, CheckoutFunnelStep, ProductSupplier, OrderStatus, RecoveryStatus, CustomerWithMetrics, CustomerSegment, Customer } from '../types';
import { generatePriceSuggestions } from './pricingService';
...
const getDateRanges = (period: 'week' | 'month') => {
    const now = new Date();
    ...
    const startPrevious = new Date(endPrevious.getTime() - days * 24 * 60 * 60 * 1000);
...
export const aiDataService = {
    ...
    // 7. NEW: Analyze Conversion Rate
    analyzeConversionRate: (ctx: AIContext, args: { period: 'week' | 'month' }) => {
        const { startCurrent, endCurrent, startPrevious, endPrevious } = getDateRanges(args.period);
        const userCheckouts = enforceUserScope(ctx.checkouts, ctx);

        const currentPeriodCheckouts = userCheckouts.filter(c => new Date(c.createdAt) >= startCurrent && new Date(c.createdAt) <= endCurrent);
        const previousPeriodCheckouts = userCheckouts.filter(c => new Date(c.createdAt) >= startPrevious && new Date(c.createdAt) <= endPrevious);

        const currentCompleted = currentPeriodCheckouts.filter(c => c.status === 'concluido').length;
        const previousCompleted = previousPeriodCheckouts.filter(c => c.status === 'concluido').length;
        
        const currentCR = currentPeriodCheckouts.length > 0 ? (currentCompleted / currentPeriodCheckouts.length) * 100 : 0;
        const previousCR = previousPeriodCheckouts.length > 0 ? (previousCompleted / previousPeriodCheckouts.length) * 100 : 0;
        
        const change = currentCR - previousCR;

        return {
            period: args.period,
            current_conversion_rate: parseFloat(currentCR.toFixed(2)),
            previous_conversion_rate: parseFloat(previousCR.toFixed(2)),
            change_percentage: parseFloat(change.toFixed(2)),
            insight: `Conversion rate changed by ${change.toFixed(2)}% compared to the previous period.`
        };
    },

    // 8. NEW: Analyze Checkout Abandonment Funnel
    analyzeCheckoutAbandonmentFunnel: (ctx: AIContext, args: { period: 'week' | 'month' }) => {
        const { startCurrent, endCurrent, startPrevious, endPrevious } = getDateRanges(args.period);
        const userCheckouts = enforceUserScope(ctx.checkouts, ctx);
        
        const currentAbandoned = userCheckouts.filter(c => c.status === 'abandonado' && new Date(c.createdAt) >= startCurrent && new Date(c.createdAt) <= endCurrent);
        const previousAbandoned = userCheckouts.filter(c => c.status === 'abandonado' && new Date(c.createdAt) >= startPrevious && new Date(c.createdAt) <= endPrevious);
        
        const countAbandonmentsByStep = (checkouts: Checkout[]) => {
            const stats: Record<CheckoutFunnelStep, number> = { 'dados_pessoais': 0, 'endereco_frete': 0, 'pagamento': 0, 'upsell': 0, 'concluido': 0 };
            checkouts.forEach(c => {
                if (c.currentStep && stats.hasOwnProperty(c.currentStep)) {
                    stats[c.currentStep]++;
                }
            });
            return stats;
        ...
        const currentStats = countAbandonmentsByStep(currentAbandoned);
        const previousStats = countAbandonmentsByStep(previousAbandoned);
        
        return {
            period: args.period,
            current_period_abandonments: {
                total: currentAbandoned.length,
                by_step: currentStats
            },
            previous_period_abandonments: {
                total: previousAbandoned.length,
                by_step: previousStats
            },
            insight: `Analysis of abandonment points for the period.`
        };
    },

    // 9. NEW: Price Optimizer Summary
    summarizePricingOpportunities: (ctx: AIContext) => {
        const suggestions = generatePriceSuggestions(ctx.products, ctx.orders, ctx.abandonmentLogs, ctx.productSuppliers);
        const opportunities = suggestions.filter(s => s.suggestedPrice.toFixed(2) !== s.currentPrice.toFixed(2));

        if (opportunities.length === 0) {
            return "Nenhuma oportunidade clara de otimização de preço foi encontrada no momento, com base nos dados atuais.";
        }
        
        const summary = `Encontrei ${opportunities.length} oportunidade(s) de otimização de preço. Sugiro que você navegue para a tela 'Otimizador de Preços' para ver os detalhes e aplicar as mudanças.`;
        return summary;
    },

    // 10. NEW: Ad Creative Helper
    generateAdCreative: (ctx: AIContext, args: { product_name: string }) => {
        const productNameLower = args.product_name.toLowerCase();
        const product = enforceUserScope(ctx.products, ctx).find(p => p.name.toLowerCase().includes(productNameLower));

        if (!product) {
            return { error: `Produto "${args.product_name}" não encontrado.` };
        }

        // Return just enough info for the LLM to work with
        return {
            name: product.name,
            category: product.category,
            salePrice: product.salePrice,
            // In a real app, you'd pull a short description from the product details.
            description: `Um produto na categoria ${product.category} vendido por R$${product.salePrice}.`
        };
    },
    
    // 11. NEW: CRM Get Segment
    getCustomerSegment: (ctx: AIContext, args: { segment: CustomerSegment, limit?: number }) => {
        const customerMetrics: CustomerWithMetrics[] = enforceUserScope(ctx.customers, ctx).map(customer => {
            const customerOrders = enforceUserScope(ctx.orders, ctx).filter(o => o.customerId === customer.id);
        ...
        const filteredCustomers = customerMetrics.filter(c => c.segment === args.segment);
        
        return filteredCustomers.slice(0, args.limit || 5).map(c => ({
            name: c.name,
            email: c.email,
            recency: c.recency,
...
export const aiActionService = {
    ...
    updateProduct: (ctx: AIContext, actionCtx: AIActionContext, args: { name_or_sku: string, field: 'price' | 'stock' | 'status', value: string }) => {
        const search = args.name_or_sku.toLowerCase();
        const product = enforceUserScope(ctx.products, ctx).find(p => p.id === args.name_or_sku || p.name.toLowerCase().includes(search) || p.sku?.toLowerCase().includes(search));
        
        if (!product) {
            return { success: false, message: `Produto "${args.name_or_sku}" não encontrado.` };
        }
        ...
        if (args.field === 'price') {
            oldValue = updatedProduct.salePrice;
            updatedProduct.salePrice = parseFloat(args.value);
        } else if (args.field === 'stock') {
            oldValue = updatedProduct.currentStock;
            updatedProduct.currentStock = parseInt(args.value, 10);
        } else if (args.field === 'status') {
            oldValue = updatedProduct.status;
            const valueLower = args.value.toLowerCase();
            if (valueLower === 'active' || valueLower === 'inactive') {
                updatedProduct.status = (valueLower.charAt(0).toUpperCase() + valueLower.slice(1)) as 'Active' | 'Inactive';
            } else {
                return { success: false, message: `Status inválido: ${args.value}. Use 'Active' ou 'Inactive'.`};
            }
        } else {
            return { success: false, message: `Campo "${args.field}" desconhecido.`};
        }
        
        actionCtx.updateProduct(updatedProduct);

        return { success: true, message: `Produto "${product.name}" atualizado. Campo '${args.field}' alterado de '${oldValue}' para '${args.value}'.` };
    },
    updateOrderStatus: (ctx: AIContext, actionCtx: AIActionContext, args: { order_id: string, new_status: OrderStatus }) => {
        const order = enforceUserScope(ctx.orders, ctx).find(o => o.id === args.order_id);
        if (!order) {
            return { success: false, message: `Pedido com ID ${args.order_id} não encontrado.` };
        }

        const oldStatus = order.status;
        const updatedOrder = { ...order, status: args.new_status };

        actionCtx.updateOrder(updatedOrder);

        return { success: true, message: `Status do pedido #${order.id.slice(0, 8)} atualizado de "${oldStatus}" para "${args.new_status}".` };
    },
    sendRecoveryMessage: (ctx: AIContext, actionCtx: AIActionContext, args: { target: 'single' | 'bulk_cart' | 'bulk_checkout', id?: string, period?: string }) => {
        if (args.target === 'single') {
            if (!args.id) return { success: false, message: "ID do log de abandono é necessário para alvo 'single'." };
            
            const log = ctx.abandonmentLogs.find(l => l.id === args.id);
            if (!log) return { success: false, message: `Log de abandono ${args.id} não encontrado.` };

            if (log.recoveryStatus !== 'pendente') return { success: false, message: `Log ${args.id} não está mais pendente.` };
            
            // Simulate sending and update status
            actionCtx.updateAbandonmentLog(log.id, { recoveryStatus: 'em_contato' as RecoveryStatus });
            return { success: true, message: `Mensagem de recuperação enviada para ${log.customerName || 'Cliente'}. Status atualizado para 'Em Contato'.` };

        } else if (args.target.startsWith('bulk')) {
            const type = args.target === 'bulk_cart' ? 'CART_ABANDONED' : 'CHECKOUT_ABANDONED';
            let logs = filterByDate(ctx.abandonmentLogs, 'abandonedAt', args.period || 'today');
            logs = logs.filter(l => l.type === type && l.recoveryStatus === 'pendente' && l.contact);

            if (logs.length === 0) {
                return { success: true, message: `Nenhum ${type === 'CART_ABANDONED' ? 'carrinho' : 'checkout'} abandonado pendente encontrado para o período.` };
            }
            ...
            return { success: true, message: `${logs.length} mensagens de recuperação foram enviadas em lote. Status atualizado para 'Em Contato'.` };
        }
