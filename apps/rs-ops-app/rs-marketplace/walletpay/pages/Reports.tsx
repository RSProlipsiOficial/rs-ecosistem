...
import { MARKETING_PLAN_DATA, MOCK_LEDGER_ENTRIES, MOCK_USER_PROFILE, typeLabels } from '../constants';
...
const EarningsSummaryTab: React.FC<{ earningsData: LedgerEntry[] }> = ({ earningsData }) => {
    ...
    const summary = useMemo(() => {
        ...
        const earnings = filteredBySearch.filter(e => e.amount > 0 && (e.type === LedgerEventType.BONUS || e.type.startsWith('commission')));
        const bonuses = earnings.filter(e => e.type === LedgerEventType.BONUS);
        const commissions = earnings.filter(e => e.type.startsWith('commission'));
...
const Reports: React.FC = () => {
    ...
    const filteredEntries = useMemo(() => {
        return MOCK_LEDGER_ENTRIES.filter(entry => {
