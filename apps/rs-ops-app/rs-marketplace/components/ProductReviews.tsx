        ...
        <div>
            ...
            <div className="my-4 flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex items-center">
                    <div className="flex">
                        {[...Array(5)].map((_, i) => (
                            <StarIcon key={i} className={`h-6 w-6 ${i < Math.round(product.rating) ? 'text-gold-400' : 'text-gray-600'}`} />
                        ))}
                    </div>
                    <p className="ml-3 text-lg font-semibold text-white">{product.rating.toFixed(1)} de 5</p>
                </div>
                {product.reviewCount > 0 && (
                    <p className="text-gray-400 text-sm sm:border-l sm:border-dark-700 sm:pl-4">Baseado em {product.reviewCount} {product.reviewCount > 1 ? 'avaliações' : 'avaliação'}</p>
                )}
            </div>
