...
const sql = `-- 1. Atualizar get_admin_user_team_usage para ser Robusto
...
async function apply() {
    try {
        console.log('--- Iniciando aplicação das correções SQL ---');
        await client.connect();
        console.log('Conectado ao banco de dados Supabase.');

        await client.query(sql);
        console.log('Sucesso: As funções RPC foram atualizadas com sucesso.');

        // Verificação rápida
        console.log('Verificando resumo de faturamento...');
        const res = await client.query('SELECT public.get_admin_billing_summary() as billing');
        console.log('Resultado da faturamento:', JSON.stringify(res.rows[0].billing, null, 2));
