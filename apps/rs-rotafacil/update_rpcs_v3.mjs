    ...
    WITH user_counts AS (
        SELECT 
            ...
            (
                ...
                AND s.status::text IN ('active', 'ativo', 'pago', 'paid', 'approved', 'confirmed')
...
BEGIN
    -- MRR: Total de planos com pagamento confirmado
    SELECT COALESCE(SUM(p.price), 0) INTO v_mrr
    FROM public.user_subscriptions s
    JOIN public.subscription_plans p ON s.plan_id = p.id
    WHERE s.status::text IN ('active', 'ativo', 'pago', 'paid', 'approved', 'confirmed');

    -- Total Revenue (Últimos 30 dias)
    SELECT COALESCE(SUM(amount), 0) INTO v_total_revenue
    FROM public.payment_history
    WHERE payment_status::text IN ('approved', 'paid', 'concluido', 'authorized', 'confirmed')
    ...
    WHERE (description::text ILIKE '%IA%' OR description::text ILIKE '%Crédito%')
      AND payment_status::text IN ('approved', 'paid', 'concluido', 'authorized', 'confirmed')
      AND created_at > now() - interval '30 days';

    -- Active Subs
    SELECT COUNT(DISTINCT user_id) INTO v_active_subs
    FROM public.user_subscriptions
    WHERE status::text IN ('active', 'ativo', 'pago', 'paid', 'approved', 'confirmed');

    -- Churn
    SELECT 
        CASE 
            WHEN COUNT(*) = 0 THEN 0 
            ELSE (SELECT COUNT(*) FROM public.user_subscriptions WHERE status::text = 'canceled' AND updated_at > now() - interval '30 days')::NUMERIC * 100 / COUNT(*)::NUMERIC 
...
BEGIN
    ...
    IF v_is_admin THEN
        ...
        FROM (
            SELECT 
                ...
                COALESCE(u.raw_user_meta_data->>'status', 'pendente') as status,
                ...
                (
                  -- Buscar o status REAL da assinatura para consistência
    ...
    ELSE
        ...
        FROM (
            ...
            FROM auth.users u
            WHERE 
                u.raw_user_meta_data->>'sponsor_id' = auth.uid()::text OR
                u.raw_user_meta_data->>'boss_id' = auth.uid()::text OR
                u.raw_user_meta_data->>'created_by' = auth.uid()::text OR
                u.id = auth.uid()
            ORDER BY u.created_at DESC
        ) u;
    END IF;
...
async function apply() {
    try {
        console.log('--- Aplicando atualização dos RPCs Admin v3 ---');
        await client.connect();
        await client.query(sql);
        console.log('Sucesso: RPCs atualizados.');
    } catch (err) {
