...
import { MonitoraSidebar } from "./monitora-sidebar";
import { Loader2 } from "lucide-react";

interface MonitoraLayoutProps {
    children: React.ReactNode;
}

export function MonitoraLayout({ children }: MonitoraLayoutProps) {
    const [sidebarCollapsed, setSidebarCollapsed] = useState(() => {
        const saved = localStorage.getItem('sidebar_collapsed_monitora');
        return saved === 'true';
    });

    useEffect(() => {
        localStorage.setItem('sidebar_collapsed_monitora', sidebarCollapsed.toString());
    }, [sidebarCollapsed]);
    ...
    useEffect(() => {
        ...
        const verifyAccess = async (session: any) => {
            try {
                ...
                // Apenas monitoras podem acessar
                if (effectiveRole !== 'monitora' && userType !== 'admin' && userType !== 'master') {
                    navigate('/auth');
                    return;
                }
                ...
                console.error('[MonitoraLayout] Erro ao verificar acesso:', error);
                navigate('/auth');
            } finally {
    ...
    return (
        <div className="min-h-screen bg-black-primary text-foreground">
            <div className="flex relative">
                <MonitoraSidebar collapsed={sidebarCollapsed} onToggle={toggleSidebar} />
