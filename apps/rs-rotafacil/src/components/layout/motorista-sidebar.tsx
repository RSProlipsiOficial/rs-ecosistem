...
const menuItems = [
    { id: "motorista", label: "Checklist Diário", icon: ClipboardList, path: "/motorista" },
    { id: "vistoria", label: "Vistoria", icon: CheckCircle, path: "/motorista?tab=checklist" },
    { id: "status", label: "Status Diário", icon: BarChart3, path: "/motorista?tab=resumo" },
    { id: "historico", label: "Histórico", icon: History, path: "/motorista?tab=historico" },
    { id: "manutencao", label: "Manutenção", icon: Wrench, path: "/motorista?tab=manutencao" },
    { id: "rota", label: "Rota", icon: Users, path: "/motorista?tab=rota" },
...
export function MotoristaSidebar({ collapsed, onToggle }: MotoristaSidebarProps) {
    ...
    useEffect(() => {
        const getUserData = async () => {
            ...
            const name = profile?.nome_completo || user.email?.split('@')[0] || 'Motorista';
            setUserName(name);
        };
        getUserData();
    }, []);
            ...
            <div className={cn(
                ...
                <nav className="flex-1 p-4 space-y-2 overflow-y-auto scrollbar-thin scrollbar-thumb-gold/20 scrollbar-track-transparent">
                    {!collapsed && menuItems.map((item) => {
                        ...
                        // Detectar se o item está ativo baseado no path e query params
                        const currentFullPath = location.pathname + location.search;
                        const isActive = currentFullPath === item.path || location.pathname === item.path;
                        return (
                ...
                {!collapsed && (
                    <div className="mt-auto border-t border-border/50 p-4 bg-black/20 animate-in fade-in duration-500">
                        ...
                        <div id="pwa-install-container" className="mt-4 hidden animate-in fade-in slide-in-from-bottom-2 duration-500">
                            <Button
                                variant="default"
                                className="w-full bg-gold text-black-primary font-black uppercase text-[10px] tracking-widest hover:bg-gold/90 h-10 shadow-gold-sm"
                                onClick={() => {
                                    const prompt = (window as any).beforeinstallpromptevent;
                                    if (prompt) {
                                        prompt.prompt();
