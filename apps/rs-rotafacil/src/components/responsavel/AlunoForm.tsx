                        ...
                        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                            ...
                            <div className="space-y-1">
                                ...
                                >
                                    ...
                                    <optgroup label="Ensino Fundamental">
                                        {Array.from({ length: 9 }, (_, i) => `${i + 1}º Ano`).map(item => (
                                            <option key={item} value={item}>{item}</option>
                                        ))}
                                    </optgroup>
                                    <optgroup label="Ensino Médio">
                                        {Array.from({ length: 3 }, (_, i) => `${i + 1}º Médio`).map(item => (
                                            <option key={item} value={item}>{item}</option>
                                        ))}
                                    </optgroup>
                                </select>
                            ...
                            <div className="space-y-1">
                                <Label className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest flex items-center gap-1">
                                    Colégio {colegiosLoading && <Loader2 className="w-2 h-2 animate-spin" />}
                                </Label>
                                <select
                                ...
                                >
                                    ...
                                    {/* Caso o dado atual seja manual e não esteja na lista, mostra ele com erro */}
                                    {formData.nome_colegio && !colegios.some(c => c.nome === formData.nome_colegio) && (
                                        <option value={formData.nome_colegio} className="text-red-400 font-bold">
                                            {formData.nome_colegio} (Pendente Regularização)
                                        </option>
                                    )}
