...
// @ts-ignore: Tipos Deno
serve(async (req: Request) => {
  const requestId = Math.random().toString(36).substring(7);
  // Verificação de cabeçalho de diagnóstico
  const isDiagnostic = req.headers.get('x-diagnostic-secret') === 'antigravity-diag-2024';
  console.log(`[${requestId}] Requisição ${req.method} para admin-users (Diagnóstico: ${isDiagnostic})`);
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'migrate-user' && userId) {
        ...
        const currentMeta = (targetUser.user.user_metadata || {}) as any;
        const { error: updateError } = await serviceRoleClient.auth.admin.updateUserById(userId, {
          user_metadata: {
            ...currentMeta,
            app: 'rotafacil',
            sponsor_id: (body as any).sponsor_id || currentMeta.sponsor_id,
            boss_id: (body as any).sponsor_id || currentMeta.boss_id,
            status: 'ativo'
          }
        });
      ...
      // 🔐 NOVA AÇÃO: Reset de Senha (via e-mail)
      if (action === 'reset-password' && targetEmail) {
        const frontendUrl = req.headers.get('origin') || 'https://rsprolipsi.com.br';
        logAction('requesting_password_reset', { targetEmail, requestId });
    ...
    if (req.method === 'PUT') {
      ...
      if (password) {
        console.log(`[${requestId}] Updating password for user ${userId} and confirming email`);
        updateData.password = password;
        updateData.email_confirm = true; // Força confirmação do e-mail se o admin está mudando a senha
      }
      ...
      if (updateError) {
        console.error(`[${requestId}] Error updating user ${userId}:`, JSON.stringify(updateError));
        throw updateError;
      }

      console.log(`[${requestId}] User ${userId} updated successfully. Password changed: ${!!password}`);
    ...
    return new Response(JSON.stringify({
      error: error.message,
      requestId,
      stack: (isDiagnostic as boolean) ? error.stack : undefined
    }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 400,
    })
