-- ==========================================================
-- SCRIPT DE CORREÇÃO: PRECISÃO DE DADOS & CONTAGEM DE EQUIPE
-- ==========================================================

-- 1. Atualizar get_admin_user_team_usage para ser Robusto (Fix OR Precedence & UUID comparison)
    ...
    WITH user_counts AS (
        SELECT 
            ...
            (
                ...
                AND s.status IN ('active', 'ativo', 'pago', 'paid')
...
-- 2. Atualizar get_admin_billing_summary para detectar vendas reais corretamente
...
BEGIN
    ...
    -- Incluindo usuários que são Rota Fácil oficial ou tem plano ativo
    SELECT COALESCE(SUM(p.price), 0) INTO v_mrr
    FROM public.user_subscriptions s
    JOIN public.subscription_plans p ON s.plan_id = p.id
    WHERE s.status IN ('active', 'ativo', 'pago', 'paid');

    -- Revenue: Total payments in last 30 days (somente aprovados/pagos)
    SELECT COALESCE(SUM(amount), 0) INTO v_total_revenue
    FROM public.payment_history
    WHERE status IN ('approved', 'paid', 'concluido', 'authorized', 'confirmed')
    ...
    WHERE (description::text ILIKE '%IA%' OR description::text ILIKE '%Crédito%')
      AND status IN ('approved', 'paid', 'concluido', 'authorized', 'confirmed')
      AND created_at > now() - interval '30 days';

    -- Active Subscriptions count (Apps Vendidos)
    -- Consideramos "Vendido" se o usuário tem um plano ativo ou o status é ativo
    SELECT COUNT(DISTINCT user_id) INTO v_active_subs
    FROM public.user_subscriptions
    WHERE status IN ('active', 'ativo', 'pago', 'paid');
...
-- 3. Atualizar get_admin_users_list para ser consistente com as contagens
