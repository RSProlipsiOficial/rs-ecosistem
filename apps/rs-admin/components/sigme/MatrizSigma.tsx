...
import { sigmaConfigAPI } from '../../services/api';
import { mockDirects, mockMatrixMembers, mockCycleInfo, mockUser } from './data';
...
const MatrizSigma: React.FC = () => {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        // Em um cenário real, o ID viria do usuário logado ou selecionado
        const targetId = '89c000c0-7a39-4e1e-8dee-5978d846fa89'; // Root como exemplo
        const res = await sigmaConfigAPI.getMatrixTree(targetId, 3);
        if (res.data?.tree) {
          setRootNode(res.data.tree);
        }
      } catch (err) {
        console.error('Falha ao carregar árvore da matriz:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const report = useMemo(() => {
    if (!rootNode) return analyzeNetwork({} as any);
  ...
  useEffect(() => {
    ...
    async function loadTree() {
      try {
        ...
        // Determine target ID and depth based on type
        if (type === 'matriz' && matrixId) {
          // ID root padrão se não for especificado
          targetId = '034051dc-8742-4df4-85ff-36a01844c612'; // João Miranda como root exemplo
          depth = 3;
        } else if (type === 'bonus-profundidade') {
          targetId = '034051dc-8742-4df4-85ff-36a01844c612';
          depth = 6;
        ...
        if (targetId) {
          const res = await sigmaConfigAPI.getMatrixTree(targetId, depth);
          if (res.data?.tree) {
            setRootNode(res.data.tree);
          }
        }
      } catch (err) {
        console.error('Erro ao carregar árvore interactiva:', err);
      } finally {
        setLoading(false);
      }
    }
    ...
    loadTree();
  }, [type, matrixId, propRootNode]);

  if (loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-12 space-y-4">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-brand-gold"></div>
        <p className="text-gray-400">Carregando estrutura real da matriz...</p>
      </div>
    );
  }

  let effectiveRootNode = rootNode;
  let effectiveTitle = propTitle;
  let effectiveMatrixWidth = propMatrixWidth || 6;

  if (!effectiveTitle) {
    switch (type) {
      case 'matriz': effectiveTitle = 'Árvore Interativa - Matriz Sigma'; break;
      case 'bonus-profundidade': effectiveTitle = 'Árvore Interativa - Bônus de Profundidade 6x6'; break;
      case 'bonus-fidelidade': effectiveTitle = 'Árvore Interativa - Bônus de Fidelidade 6x6'; break;
      default: effectiveTitle = 'Árvore Interativa';
    }
  }
