...
import { MINISITE_URL } from '../../src/config/urls';
import {
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'dashboard':
        return (
          <div className="space-y-6 animate-fade-in">
            <h2 className="text-3xl font-sans font-bold text-white">Visão Geral Global</h2>

            {/* KPI Cards */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-[#1E1E1E] p-6 rounded-xl border border-[#2A2A2A] shadow-sm">
                ...
                <p className="text-sm text-[#9CA3AF]">Receita Total (MRR)</p>
                <h3 className="text-2xl font-bold text-white mt-1">R$ {totalRevenue.toLocaleString('pt-BR')}</h3>
              </div>
              ...
              <div className="bg-[#1E1E1E] p-6 rounded-xl border border-[#2A2A2A] shadow-sm">
                ...
                <p className="text-sm text-[#9CA3AF]">Agências Ativas</p>
                <h3 className="text-2xl font-bold text-white mt-1">{agencies.length}</h3>
              </div>
              ...
              <div className="bg-[#1E1E1E] p-6 rounded-xl border border-[#2A2A2A] shadow-sm">
                ...
                <p className="text-sm text-[#9CA3AF]">Clientes Finais</p>
              ...
              <div className="bg-[#1E1E1E] p-6 rounded-xl border border-[#2A2A2A] shadow-sm">
                ...
                <p className="text-sm text-[#9CA3AF]">MiniSites Publicados</p>
                <h3 className="text-2xl font-bold text-white mt-1">{totalSites}</h3>
              </div>
            </div>
            ...
            <div className="bg-[#1E1E1E] rounded-xl border border-[#2A2A2A] overflow-hidden">
              ...
              <div className="divide-y divide-[#2A2A2A]">
                {payments.slice(0, 5).map(payment => (
                  <div key={payment.id} className="p-4 flex items-center justify-between hover:bg-[#1E1E1E] dark:hover:bg-white/5">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#9CA3AF]">{new Date(payment.date).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <span className="font-bold text-green-600 dark:text-green-400">+ R$ {payment.amount.toFixed(2)}</span>
                  </div>
                ))}
                {payments.length === 0 && <div className="p-6 text-center text-[#9CA3AF]">Sem dados financeiros.</div>}
              </div>
            </div>
          </div>
        );
      ...
      case 'walletpay':
        // Cálculos para o extrato
        const paidPayments = payments.filter(p => p.status === 'paid');
        const totalSales = paidPayments.reduce((acc, p) => acc + p.amount, 0);
        const totalCommission = totalSales * 0.10; // 10% de comissão
        const netAmount = totalSales - totalCommission;

        return (
          <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-3xl font-bold text-white">Extrato Financeiro</h2>
              <div className="flex gap-4">
                <div className="bg-[#1E1E1E] p-4 rounded-xl border border-[#2A2A2A] min-w-[150px]">
                  <p className="text-[10px] text-[#9CA3AF] uppercase font-bold mb-1">Total Vendas</p>
                  <p className="text-xl font-bold text-white">R$ {totalSales.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                </div>
                <div className="bg-[#1E1E1E] p-4 rounded-xl border border-[#2A2A2A] min-w-[150px]">
                  <p className="text-[10px] text-[#9CA3AF] uppercase font-bold mb-1">Comissão (10%)</p>
                  <p className="text-xl font-bold text-[#FFD700]">R$ {totalCommission.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                </div>
                <div className="bg-[#1E1E1E] p-4 rounded-xl border border-[#2A2A2A] min-w-[150px]">
                  <p className="text-[10px] text-[#9CA3AF] uppercase font-bold mb-1">Valor Líquido</p>
                  <p className="text-xl font-bold text-green-500">R$ {netAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                </div>
              </div>
            </div>

            <div className="bg-[#1E1E1E] rounded-xl border border-[#2A2A2A] overflow-hidden">
              <div className="p-4 border-b border-[#2A2A2A] flex justify-between items-center bg-[#121212]/30">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead className="bg-[#121212] text-[#9CA3AF] font-bold uppercase text-[10px] tracking-wider">
                    <tr>
                      <th className="p-4">Data</th>
                      <th className="p-4">Cliente / Agência</th>
                      <th className="p-4 text-center">Valor Bruto</th>
                      <th className="p-4 text-center">Comissão (10%)</th>
                      <th className="p-4 text-center">Líquido</th>
                    ...
                    {payments.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(payment => {
                      ...
                      return (
                        <tr key={payment.id} className="hover:bg-[#2A2A2A] transition-colors">
                          ...
                          <td className="p-4">
                            <div className="font-bold text-white">{client?.name || 'Cliente Desconhecido'}</div>
                            <div className="text-[10px] text-[#9CA3AF] uppercase">Agência: {agency?.name || 'Independente'}</div>
                          </td>
                          <td className="p-4 text-center font-mono text-white">
                            R$ {payment.amount.toFixed(2).replace('.', ',')}
                          </td>
                          <td className="p-4 text-center font-mono text-[#FFD700]">
                            R$ {(payment.amount * 0.1).toFixed(2).replace('.', ',')}
                          </td>
                          <td className="p-4 text-center font-mono text-green-500">
                            R$ {(payment.amount * 0.9).toFixed(2).replace('.', ',')}
                          </td>
                          <td className="p-4 text-center">
                            <span className={`text-[10px] px-2 py-1 rounded border uppercase font-bold ${payment.status === 'paid' ? 'border-green-500 text-green-500 bg-green-500/10' : 'border-yellow-500 text-yellow-500 bg-yellow-500/10'
                              }`}>
        ...
        return (
          <div className="space-y-6 animate-fade-in relative">
            <div className="flex justify-between items-center">
              <h2 className="text-3xl font-sans font-bold text-white">Agências Parceiras</h2>
              <button
                onClick={handleOpenNewAgency}
                className="bg-rs-gold text-black hover:bg-[#E5C100] text-black px-4 py-2 rounded-lg font-bold text-sm hover:opacity-90 flex items-center gap-2"
              >
                <Plus size={16} />
                Nova Agência
              </button>
            ...
            <div className="bg-[#1E1E1E] rounded-xl border border-[#2A2A2A] overflow-hidden">
              <table className="w-full text-sm text-left">
                <thead className="bg-[#1E1E1E] text-[#9CA3AF] font-bold uppercase text-xs">
                  <tr>
                ...
                <tbody className="divide-y divide-[#2A2A2A]">
                  {agencies.map(agency => (
                    <tr key={agency.id} className="hover:bg-[#1E1E1E] dark:hover:bg-white/5">
                      <td className="p-4 font-bold text-white">{agency.name}</td>
                      <td className="p-4 text-[#9CA3AF]">{agency.email}</td>
                      <td className="p-4 text-center">
                      ...
                      <td className="p-4 text-right text-white font-mono">R$ {(clients.filter(c => c.agencyId === agency.id).reduce((acc, c) => acc + (c.monthlyFee || 0), 0)).toFixed(2)}</td>
                      <td className="p-4 text-center">
                        <span className={`text-[10px] px-2 py-1 rounded border uppercase font-bold ${agency.status === 'active' ? 'border-green-500 text-green-500' : 'border-gray-500 text-[#9CA3AF]'
                          }`}>
                  ...
                  {agencies.length === 0 && (
                    <tr><td colSpan={6} className="p-8 text-center text-[#9CA3AF]">Nenhuma agência encontrada.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            {isAgencyModalOpen && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
                <div className="w-full max-w-lg bg-[#1E1E1E] border border-[#2A2A2A] dark:border-rs-gold/30 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
                  <div className="p-6 border-b border-[#2A2A2A] dark:border-[#2A2A2A] flex justify-between items-center bg-[#1E1E1E] shrink-0">
                  ...
                  <div className="overflow-y-auto p-6">
                    <form onSubmit={handleSaveAgency} className="space-y-4">
                      <div>
                        <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Nome da Agência</label>
                        <input
                          type="text"
                          required
                          className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                          value={agencyForm.name}
                          onChange={e => setAgencyForm({ ...agencyForm, name: e.target.value })}
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">E-mail Administrativo</label>
                        <input
                          type="email"
                          required
                          className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Status</label>
                          <select
                            className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                        ...
                        <div>
                          <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Faturamento (R$)</label>
                          <input
                            type="number"
                            min="0"
                            className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                      ...
                      <div className="pt-4 flex gap-3 justify-end">
                        <button
                          ...
                          className="px-4 py-2 rounded-lg text-[#9CA3AF] hover:text-white dark:hover:text-white font-bold text-sm"
                        >
                          Cancelar
                        </button>
                        <button
                          type="submit"
                          className="px-6 py-2 bg-rs-gold text-black hover:bg-[#E5C100] text-black rounded-lg font-bold text-sm hover:opacity-90 flex items-center gap-2"
                        >
                          <Save size={16} />
                          Salvar Alterações
                        </button>
      ...
      case 'clients':
        return (
          <div className="space-y-6 animate-fade-in relative">
            <div className="flex justify-between items-center">
              <h2 className="text-3xl font-sans font-bold text-white">Clientes Finais</h2>
              <button
              ...
              <div className="p-4 border-b border-[#2A2A2A] flex gap-4">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-[#1E1E1E] border border-[#2A2A2A] rounded-lg outline-none focus:border-rs-gold text-sm"
                    value={searchTerm}
                ...
                <tbody className="divide-y divide-[#2A2A2A]">
                  {clients
                    ...
                    .map(client => {
                      ...
                      return (
                        <tr key={client.id} className="hover:bg-[#1E1E1E] dark:hover:bg-white/5">
                          <td className="p-4">
                          ...
                          <td className="p-4">
                            {responsibleAgency ? (
                              <span className="text-xs bg-[#2A2A2A] px-2 py-1 rounded text-[#E5E7EB] border border-[#2A2A2A] dark:border-gray-700">
                          ...
                          <td className="p-4 text-center">
                            <span className={`text-[10px] px-2 py-1 rounded border uppercase font-bold ${client.status === 'active' ? 'border-green-500 text-green-500' : 'border-gray-500 text-[#9CA3AF]'
                              }`}>
                  ...
                  {clients.length === 0 && (
                    <tr><td colSpan={6} className="p-8 text-center text-[#9CA3AF]">Nenhum cliente cadastrado.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            {isClientModalOpen && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
                <div className="w-full max-w-2xl bg-[#1E1E1E] border border-[#2A2A2A] dark:border-rs-gold/30 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                  <div className="p-6 border-b border-[#2A2A2A] dark:border-[#2A2A2A] flex justify-between items-center bg-[#1E1E1E] shrink-0">
                    <h3 className="font-sans font-bold text-xl text-white">
                  ...
                  <div className="overflow-y-auto p-6 custom-scrollbar">
                    <form onSubmit={handleSaveClient} className="space-y-6">
                      ...
                      {/* Financial Section */}
                      <div className="bg-[#1E1E1E] dark:bg-white/5 border border-[#2A2A2A] dark:border-white/10 rounded-lg p-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Valor Mensalidade (R$)</label>
                            <input
                              type="number"
                              min="0"
                              step="0.01"
                              className="w-full bg-white dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold font-bold"
                      ...
                      <div className="space-y-4">
                        <h4 className="text-xs font-bold text-rs-goldDark dark:text-rs-gold uppercase tracking-widest border-b border-[#2A2A2A] pb-2">Dados Pessoais</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="col-span-2 md:col-span-1">
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">E-mail</label>
                            <input
                              type="email"
                              required
                              className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                          ...
                          <div className="col-span-2 md:col-span-1">
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Telefone / WhatsApp</label>
                            <input
                              type="text"
                              className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                          ...
                          <div className="col-span-2 md:col-span-1">
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Data de Nascimento</label>
                            <input
                              type="date"
                              className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                      ...
                      {/* Section 2: Address */}
                      <div className="space-y-4">
                        <h4 className="text-xs font-bold text-rs-goldDark dark:text-rs-gold uppercase tracking-widest border-b border-[#2A2A2A] pb-2">Endereço</h4>
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                          <div className="col-span-1">
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">CEP</label>
                            <input
                              type="text"
                              className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                          ...
                          <div className="col-span-3">
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Logradouro (Rua, Av)</label>
                            <input
                              type="text"
                              className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                          ...
                          <div className="col-span-1">
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Número</label>
                            <input
                              type="text"
                              className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                          ...
                          <div className="col-span-2">
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Cidade</label>
                            <input
                              type="text"
                              className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                          ...
                          <div className="col-span-1">
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Estado</label>
                            <input
                              type="text"
                              className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                      ...
                      <div className="space-y-4">
                        <h4 className="text-xs font-bold text-rs-goldDark dark:text-rs-gold uppercase tracking-widest border-b border-[#2A2A2A] pb-2">Gestão</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Agência Responsável</label>
                            <select
                              className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                          ...
                          <div className="col-span-2">
                            <label className="block text-xs font-bold text-[#9CA3AF] uppercase mb-1">Notas Internas</label>
                            <textarea
                              className="w-full bg-[#1E1E1E] dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg p-3 text-white outline-none focus:border-rs-gold"
                              rows={3}
      ...
      case 'packages':
        return (
          <div className="space-y-6 animate-fade-in text-white">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-3xl font-sans font-bold dark:text-white">Kits & Pacotes RS</h2>
                <p className="text-[#9CA3AF]">Sincronização automática com checkout seguro.</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {['Start', 'Pro', 'Agente'].map((kit) => (
                <div key={kit} className="bg-[#1E1E1E] p-6 rounded-xl border border-rs-gold/20 flex flex-col justify-between">
                  <div>
                    <h3 className="text-xl font-bold mb-2">Kit RS {kit}</h3>
                    <p className="text-sm text-[#9CA3AF] mb-4">Este kit usa automaticamente as credenciais do Mercado Pago configuradas.</p>
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2 text-xs text-blue-500 bg-blue-500/10 p-2 rounded">
                        <Globe size={14} /> Link de Venda: {MINISITE_URL}/checkout/{kit.toLowerCase()}
                      </div>
                    </div>
                  </div>
      ...
      case 'sites':
        return (
          <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
              <h2 className="text-3xl font-sans font-bold text-white">MiniSites por Dono</h2>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                <input
                  type="text"
                  placeholder="Buscar sites..."
                  className="pl-10 pr-4 py-2 bg-[#1E1E1E] border border-[#2A2A2A] rounded-lg outline-none focus:border-rs-gold text-sm w-64"
                  value={searchTerm}
                  onChange={e => setSearchTerm(e.target.value)}
                />
              </div>
            ...
            <div className="space-y-4">
              {sitesByOwner.map((ownerGroup) => (
                <div key={ownerGroup.id} className="bg-[#1E1E1E] rounded-xl border border-[#2A2A2A] overflow-hidden">

                  {/* Owner Header - Click to Expand */}
                  <div
                    onClick={() => setExpandedOwnerId(expandedOwnerId === ownerGroup.id ? null : ownerGroup.id)}
                    className="p-6 flex items-center justify-between cursor-pointer hover:bg-[#1E1E1E] dark:hover:bg-white/5 transition-colors"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-bold text-lg text-white">{ownerGroup.ownerName}</h3>
                        <p className="text-xs text-[#9CA3AF]">{ownerGroup.ownerType}</p>
                      </div>
                    </div>

                    <div className="flex items-center gap-8">
                      <div className="text-center">
                        <p className="text-xs text-[#9CA3AF] uppercase font-bold">Sites</p>
                        <p className="text-lg font-bold">{ownerGroup.sites.length}</p>
                      </div>
                      <div className="text-center">
                        <p className="text-xs text-[#9CA3AF] uppercase font-bold">Visualizações</p>
                  ...
                  {expandedOwnerId === ownerGroup.id && (
                    <div className="border-t border-[#2A2A2A] bg-[#1E1E1E]/50 dark:bg-black/20 p-4 animate-fade-in">
                      <table className="w-full text-sm text-left">
                        <thead className="text-[#9CA3AF] font-bold uppercase text-[10px]">
                          <tr>
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-white/5">
                          {ownerGroup.sites
                            ...
                            .map(site => (
                              <tr key={site.id} className="hover:bg-white dark:hover:bg-white/5">
                                ...
                                <td className="p-3 text-[#9CA3AF] font-mono text-xs">/{site.slug}</td>
                          ...
                          {ownerGroup.sites.filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 && (
                            <tr><td colSpan={6} className="p-4 text-center text-[#9CA3AF] text-xs">Nenhum site encontrado para esta busca.</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
      ...
      case 'plans':
        return (
          <div className="space-y-6 animate-fade-in text-white">
            <div className="flex justify-between items-center">
              <h2 className="text-3xl font-sans font-bold dark:text-white">Gerenciar Planos</h2>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Object.entries(tempPlans).filter(([key]) => key !== 'admin_master').map(([key, plan]: [string, PlanDefinition]) => (
                <div key={key} className="bg-[#1E1E1E] p-6 rounded-xl border border-[#2A2A2A] relative overflow-hidden group">
      ...
      case 'settings':
        return (
          <div className="space-y-6 animate-fade-in text-white">
            <h2 className="text-3xl font-sans font-bold dark:text-white">Configurações Globais</h2>
            <div className="bg-[#1E1E1E] p-8 rounded-xl border border-[#2A2A2A] max-w-2xl">
              <div className="space-y-6">
                ...
                <div className="pt-4 border-t border-white/10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold text-[#9CA3AF] uppercase mb-1">Mercado Pago Public Key</label>
                      <input
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-[#9CA3AF] uppercase mb-1">Mercado Pago Access Token</label>
                      <input
                        type="password"
                        className="w-full bg-black/50 border border-rs-gray p-2 rounded text-xs outline-none focus:border-rs-gold"
                ...
                <div className="pt-4">
                  <button
                    onClick={() => {
                      if (onUpdateSettings) onUpdatePlans(tempSettings);
      ...
      case 'logs':
        return (
          <div className="space-y-6 animate-fade-in">
            <h2 className="text-3xl font-sans font-bold text-white">Logs do Sistema</h2>
            <div className="bg-[#1E1E1E] rounded-xl border border-[#2A2A2A] overflow-hidden">
              <div className="p-4 border-b border-[#2A2A2A] flex justify-between items-center">
                <h3 className="font-bold text-sm text-[#9CA3AF] uppercase">Histórico de Atividades</h3>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-[#2A2A2A]">
                  {[...logs].reverse().map(log => (
                    <tr key={log.id} className="hover:bg-[#1E1E1E] dark:hover:bg-white/5">
                      <td className="p-4 text-[#9CA3AF] text-xs font-mono whitespace-nowrap">
                        {log.timestamp.toLocaleString()}
                      </td>
                      <td className="p-4">
                        <div className="flex flex-col">
                          <span className="font-bold text-white">{log.actorName}</span>
                          <span className="text-xs text-[#9CA3AF]">{log.actorEmail}</span>
                        </div>
                      </td>
                      <td className="p-4">
                  ...
                  {logs.length === 0 && (
                    <tr>
                      <td colSpan={4} className="p-8 text-center text-[#9CA3AF]">
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] bg-[#121212] overflow-hidden">
      {/* Sidebar */}
      <aside className="w-64 bg-[#121212] border-r border-[#2A2A2A] flex flex-col shrink-0 z-20">
        <div className="p-6 border-b border-[#2A2A2A] flex items-center gap-3">
          ...
          <div>
            <h1 className="font-sans font-bold text-white leading-tight">Admin Master</h1>
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1">
          <MenuButton
            ...
            label="Visão Geral"
          />
          <div className="pt-4 pb-2 px-2 text-[10px] font-bold text-[#9CA3AF] uppercase tracking-widest">Ecossistema</div>
          <MenuButton
          ...
          <div className="pt-4 mt-4 border-t border-white/5">
            <MenuButton active={activeTab === 'walletpay'} onClick={() => setActiveTab('walletpay')} icon={<DollarSign size={18} />} label="Extrato" />
      ...
      <main className="flex-1 overflow-auto relative">
        <header className="sticky top-0 z-10 bg-[#121212]/80 backdrop-blur-md border-b border-[#2A2A2A] px-8 py-4 flex justify-between items-center">
          <div className="flex items-center gap-2 text-sm text-[#9CA3AF]">
            <span>Admin</span>
            <ChevronRight size={14} />
            <span className="text-white capitalize font-bold">
              {activeTab === 'dashboard' ? 'Visão Geral' :
                activeTab === 'agencies' ? 'Agências' :
                  activeTab === 'clients' ? 'Clientes Finais' :
                    activeTab === 'sites' ? 'MiniSites' :
                      activeTab === 'packages' ? 'Pacotes' :
                        activeTab === 'plans' ? 'Planos' :
                          activeTab === 'settings' ? 'Plataforma' :
                            activeTab === 'walletpay' ? 'Extrato Financeiro' :
                              activeTab === 'logs' ? 'Logs de Sistema' : activeTab}
            </span>
          </div>
          <div className="flex items-center gap-4">
...
const MenuButton = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-bold transition-all ${active
