                ...
                <main className="p-6 overflow-y-auto">
                    ...
                    <div className="border border-gray-800 rounded-lg overflow-hidden">
                        <table className="w-full text-sm text-left text-gray-300">
                            <thead className="bg-black/30 text-xs text-uppercase text-gray-400">
                                <tr>
                            ...
                            <tbody>
                                {order.items.map((item, idx) => (
                                    <tr key={idx} className="border-t border-gray-800">
                                        ...
                                        <td className="px-3 py-2 text-right">{item.unitPrice.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
                                        <td className="px-3 py-2 text-right font-semibold text-white">{(item.qty * item.unitPrice).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
                                    </tr>
                                ))}
                            </tbody>
                            <tfoot className="bg-black/50 font-bold text-white">
                                <tr>
                                    <td colSpan={3} className="px-3 py-2 text-right">Subtotal</td>
                                    <td className="px-3 py-2 text-right">{subtotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                </main>
                <footer className="p-4 border-t border-gray-700 flex justify-end gap-3 no-print">
                    <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-300 bg-gray-800 rounded-lg hover:bg-gray-700 transition-colors" disabled={isSaving}>Fechar</button>
                    {hasChanges && (
                        <button onClick={handleSave} disabled={isSaving} className="px-4 py-2 text-sm font-bold text-black bg-yellow-500 rounded-lg hover:bg-yellow-400 transition-colors flex items-center gap-2">
...
const CDReportsPage: React.FC<CDReportsPageProps> = ({ cdId }) => {
    const [orders, setOrders] = useState<PurchaseOrder[]>(mockPurchaseOrders);
    const [activeTab, setActiveTab] = useState<ReportTab>('purchases');
    const [statusFilter, setStatusFilter] = useState('Todos os Status');
    const [paymentFilter, setPaymentFilter] = useState('Todos os Pagamentos');
    const [selectedOrder, setSelectedOrder] = useState<PurchaseOrder | null>(null);

    const filteredOrders = useMemo(() => {
        return orders.filter(order => {
            if (statusFilter !== 'Todos os Status' && order.status !== statusFilter) return false;
            if (paymentFilter !== 'Todos os Pagamentos' && order.paymentMethod !== paymentFilter) return false;
            return true;
        });
