...
type RegistrationData = { name: string, email: string, doc: string, apiKey: string, apiSecret: string, plan: User['plan'] };
    ...
    const handleRegister = async (data: RegistrationData): Promise<{ success: boolean; message?: string }> => {
        try {
            ...
            const newUser = {
                name: data.name,
                email: data.email,
                isAdmin: false, // Changed from is_admin to isAdmin
                plan: data.plan,
                subscriptionEndDate: endDate.toISOString(), // Changed from subscription_end_date to subscriptionEndDate
                doc: data.doc,
            };
