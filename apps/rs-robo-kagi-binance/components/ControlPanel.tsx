...
import { saveConfig, verifyBinanceAccount } from '../lib/api';
...
// The global config is now very simple.
...
interface ControlPanelProps { 
    ...
    // FIX: Changed from Partial<AppConfig> to AppConfig to match the state type in App.tsx.
...
export default function ControlPanel({ 
  ...
  // FIX: Changed from Partial<AppConfig> to AppConfig to match the prop type.
  ...
  const handleLiveModeChange = async (newLiveStatus: boolean) => {
    const newCfg = { ...cfg, live: newLiveStatus };
    setCfg(newCfg);
    try {
        const data = await saveConfig(newCfg);
        onConfigSave(data);
        addToast(`Modo de operação alterado para: ${newLiveStatus ? 'Conta Real' : 'Simulação'}`, 'info');
    } catch (e) {
        if (e instanceof Error) addToast(e.message, 'error');
        setCfg(appConfig); // Revert on failure
    }
  };
