...
import { clearWalletSession, readWalletSession } from '../src/utils/walletSession';
...
const Topbar: React.FC<TopbarProps> = ({ onMenuClick }) => {
  ...
  const path = location.pathname.split('/').filter((segment) => segment && segment !== 'app')[0];
  ...
  const [userProfile, setUserProfile] = useState<{ name: string; avatarUrl: string | null }>({
    name: storedSession.userName || 'Consultor',
    avatarUrl: null
  });
      ...
      try {
        ...
        if (data) {
          setUserProfile((previous) => ({
            name: data.nome_completo || previous.name,
            avatarUrl: data.avatar_url
          }));
        }
      } catch (error) {
        console.error('Erro ao buscar dados do topo:', error);
      }
    };
        ...
        <div className="relative" ref={profileMenuRef}>
          ...
          {isProfileMenuOpen && (
            <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-lg shadow-lg bg-card border border-border ring-1 ring-black ring-opacity-5 z-20">
              <div className="py-1">
                <button onClick={() => { setProfileMenuOpen(false); navigate('/app/settings'); }} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-text-body hover:bg-surface hover:text-text-title transition-colors">
