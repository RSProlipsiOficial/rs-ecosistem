...
const Dashboard: React.FC = () => {
    ...
    const calculateSalesVolume = (src: LedgerEntry[]) => {
        const now = new Date();
        return src
            .filter(e => e.amount > 0 && (e.type === 'COMMISSION_SHOP' || e.type === 'COMMISSION_REFERRAL') && new Date(e.occurredAt).getMonth() === now.getMonth() && new Date(e.occurredAt).getFullYear() === now.getFullYear())
            .reduce((sum, e) => sum + e.amount, 0);
    };
